function validateForm() {
    ...
    let tongCong = $("#lblTongCong").text().slice(0,-4);
    console.log(ten, dienThoai, diaChi,tongCong);

    if (ten == "") {
        alert("Qúy Khách vui lòng nhập Tên!");
        $("#ten").focus();
        return false;
    }
    if (dienThoai == "") {
        alert("Qúy Khách vui lòng nhập Số Điện Thoại!");
        $("#dienThoai").focus();

        return false;
    }
    if (diaChi == "") {
        alert("Qúy Khách vui lòng nhập Địa Chỉ!");
        $("#diaChi").focus();

        return false;
    }

    if (Number(tongCong) < 200) {
        alert("Đơn hàng tối thiểu là 200k, xin quý khách đặt thêm hàng!");
        return false;
    }
...
function main() {
    ...
    function capNhatGia() {
        ...
        $("h3[name='tongcong']").html(`${tongCong} <sup> kVND</sup>`);
    }
}